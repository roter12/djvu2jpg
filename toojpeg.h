...
// void myOutput(std::ofstream *myFileHandle, unsigned char oneByte) { fputc(oneByte, myFileHandle); } // save byte to file
// => let's go !
// TooJpeg::writeJpeg(ofile, myOutput, mypixels, 1024, 768);
...
namespace TooJpeg
{
  ...
  typedef void (*WRITE_ONE_BYTE)(std::ofstream *, unsigned char);
  ...
  // auto myOutput = [](std::ofstream *myFileHandle, unsigned char oneByte) { fputc(oneByte, myFileHandle); };

  // output       - callback that stores a single byte (writes to disk, memory, ...)
  // ofile        - output file stream for output callback
  ...
  bool writeJpeg(WRITE_ONE_BYTE output, std::ofstream *ofile, const void *pixels, unsigned short width, unsigned short height,
